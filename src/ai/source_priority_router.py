"""
Source Priority Routing System - AI Implementation
Implements domain-specific routing with Singapore-first strategy for Task 3.2
Part of the AI implementation layer for intelligent query routing
"""
import logging
...
class SourcePriorityRouter:
    ...
    def _load_domain_routing_rules(self) -> Dict:
        ...
        return {
            'psychology': {
                'primary_sources': ['kaggle', 'zenodo'],  # Based on training mappings: kaggle (0.95), zenodo (0.90)
                'secondary_sources': ['world_bank'],  # Lower relevance (0.25) but included as fallback
            ...
            'climate': {
                ...
                'secondary_sources': ['zenodo', 'kaggle'],  # zenodo (0.85), kaggle (0.82)
                'singapore_sources': ['data_gov_sg'],
            ...
            'singapore': {
                'primary_sources': ['data_gov_sg', 'singstat', 'lta_datamall'],  # Official Singapore sources
            ...
            'health': {
                'primary_sources': ['world_bank', 'zenodo'],  # world_bank (0.88), zenodo (0.90)
            ...
            'education': {
                ...
                'secondary_sources': ['kaggle', 'zenodo'],  # kaggle (0.85), zenodo (0.83)
                'singapore_sources': ['data_gov_sg', 'singstat'],
                'quality_threshold': 0.8,
    ...
    def _load_training_mappings(self):
        ...
        try:
            ...
            if mappings_path.exists():
                ...
                # Parse mappings - store multiple sources per query
                lines = content.split('\n')
                for line in lines:
                    line = line.strip()
                ...
                # Sort each query's sources by relevance score
                for query in self.training_mappings:
                    self.training_mappings[query].sort(key=lambda x: -x['score'])
                
                total_mappings = sum(len(sources) for sources in self.training_mappings.values())
                logger.info(f"✅ Loaded {total_mappings} training mappings for {len(self.training_mappings)} queries")
            else:
    ...
    def route_query(self, query: str, domain: str, singapore_first: bool = False) -> List[Dict]:
        ...
        domain_rules = self.domain_routing_rules.get(domain, self.domain_routing_rules['general'])
        ...
        # Apply fallback routing if no high-quality sources found OR for truly ambiguous queries
        is_ambiguous = self._is_ambiguous_query(query, domain)
        needs_fallback = (
            not sources or 
            max(s['relevance_score'] for s in sources) < domain_rules['quality_threshold'] or
            is_ambiguous
        )
        ...
        if needs_fallback:
            ...
            if is_ambiguous:
                # For ambiguous queries, prioritize fallback sources
                existing_source_names = {s['source'] for s in sources}
                for fallback in fallback_sources:
                    if fallback['source'] not in existing_source_names:
                        sources.append(fallback)
                    else:
                        # Update existing source to show fallback reasoning
                        for existing in sources:
                            if existing['source'] == fallback['source']:
                                existing['routing_reason'] = fallback['routing_reason']
                                break
            else:
                # Normal fallback - just add missing sources
                existing_source_names = {s['source'] for s in sources}
                for fallback in fallback_sources:
                    if fallback['source'] not in existing_source_names:
                        sources.append(fallback)
    ...
    def _get_exact_training_matches(self, query: str) -> List[Dict]:
        ...
        if query_lower in self.training_mappings:
            for mapping in self.training_mappings[query_lower]:
                matches.append({
                    'source': mapping['source'],
                    'relevance_score': mapping['score'],
                    'routing_reason': 'exact_training_match',
                    'priority': 1,
                    'source_info': self.source_definitions.get(mapping['source'], {})
                })
        
        # Check for partial matches (query contains training mapping key)
        for mapping_query, mapping_list in self.training_mappings.items():
            if mapping_query != query_lower and mapping_query in query_lower:
                # Only take the best source for partial matches
                best_mapping = mapping_list[0] if mapping_list else None
                if best_mapping:
                    matches.append({
                        'source': best_mapping['source'],
                        'relevance_score': best_mapping['score'] * 0.9,  # Slightly lower for partial match
                        'routing_reason': 'partial_training_match',
                        'priority': 2,
                        'source_info': self.source_definitions.get(best_mapping['source'], {})
                    })
        
        # Sort by priority and relevance, remove duplicates
        seen_sources = set()
        unique_matches = []
        for match in sorted(matches, key=lambda x: (x['priority'], -x['relevance_score'])):
    ...
    def _apply_fallback_routing(self, query: str, domain: str, rules: Dict) -> List[Dict]:
        ...
        if fallback_strategy == 'research_platforms':
            ...
            fallback_candidates = ['data_gov_sg', 'singstat', 'lta_datamall', 'world_bank']
        ...
        for source in fallback_candidates:
            if source in self.source_definitions:
                relevance_score = self._calculate_fallback_relevance(query, source, domain)
    ...
    def _calculate_fallback_relevance(self, query: str, source: str, domain: str) -> float:
        ...
        if source == 'kaggle' and any(term in query.lower() for term in ['data', 'dataset', 'analysis']):
            base_score += 0.1
        elif source == 'world_bank' and any(term in query.lower() for term in ['global', 'country', 'statistics']):
            base_score += 0.1
        elif source == 'zenodo' and any(term in query.lower() for term in ['research', 'academic', 'study']):
            base_score += 0.1
        
        return min(base_score, 0.8)  # Cap fallback scores at 0.8
    
    def _route_singapore_first(self, query: str, domain: str, rules: Dict) -> List[Dict]:
        ...
        for source in rules['singapore_sources']:
            if source in self.source_definitions:
                sources.append({
                    'source': source,
                    'relevance_score': self._calculate_relevance(query, source, domain),
        ...
        for source in rules['primary_sources']:
            if source not in rules['singapore_sources'] and source in self.source_definitions:
                sources.append({
                    'source': source,
                    'relevance_score': self._calculate_relevance(query, source, domain),
        ...
        for source in rules['secondary_sources']:
            if source not in [s['source'] for s in sources] and source in self.source_definitions:
                sources.append({
                    'source': source,
                    'relevance_score': self._calculate_relevance(query, source, domain),
    ...
    def _calculate_relevance(self, query: str, source: str, domain: str) -> float:
        ...
        if query.lower() in self.training_mappings:
            for mapping in self.training_mappings[query.lower()]:
        ...
        if 'singapore' in query.lower() and source_info.get('singapore_priority', False):
            base_relevance += 0.2
    ...
    def _is_ambiguous_query(self, query: str, domain: str) -> bool:
        """Determine if a query is ambiguous and needs fallback routing"""
        query_lower = query.lower()
        
        # Queries with vague or generic terms
        ambiguous_terms = [
            'random', 'unknown', 'new', 'obscure', 'misc', 'various', 'different',
            'some', 'any', 'general', 'basic', 'simple', 'other', 'stuff'
        ]
        ...
        # Very short queries (likely ambiguous)
        if len(query_lower.split()) <= 2 and domain == 'general':
            return True
        
        # Queries that don't match any specific domain patterns
        if domain == 'general' and not any(
            keyword in query_lower 
            for domain_def in self.domain_routing_rules.values() 
            for keyword in domain_def.get('primary_sources', [])
        ):
            return True
...
def route_query_to_sources(query: str, domain: str, singapore_first: bool = False, config: Dict = None) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    print("🧪 Testing Source Priority Router - AI Implementation\n")
    ...
    for query, domain, singapore_first in test_cases:
        ...
        for i, source in enumerate(sources[:3], 1):
            ...
            print(f"       Description: {source['source_info'].get('description', 'N/A')}")
